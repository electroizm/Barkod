...
async function fisCacheYukle(fisNo, client, zorlaYenile = false) {
    ...
    console.log(`Giriş fişi cache yüklendi: ${fisNo} - ${kalemler?.length || 0} kalem, ${okunanQrler.size} okuma`);
...
/**
 * GET /api/giris/fis/:fisNo
 * Giriş fişi detaylarını getir (sadece Supabase)
 */
router.get('/fis/:fisNo', async (req, res) => {
    try {
        ...
        if (!data || data.length === 0) {
            return res.status(404).json({ success: false, message: 'Giriş fişi bulunamadı' });
        }
...
router.get('/fis-durumu/:fisNo', async (req, res) => {
    try {
        ...
        console.error('Giriş fişi durumu hatası:', error);
        return res.status(500).json({ success: false, message: 'Sunucu hatası: ' + error.message });
    }
});

/**
 * GET /api/giris/okunan-qrler/:fisNo
...
router.post('/qr-okut', async (req, res) => {
    try {
        ...
        console.error('Giriş fişi QR okutma hatası:', error);
        return res.json({ success: false, message: 'Sunucu hatası: ' + error.message, hata_tipi: 'SERVER_ERROR' });
    }
});

/**
 * GET /api/giris/malzeme-paketler/:fisNo/:kalemId
...
/**
 * GET /api/giris/kapatilan-fisler
