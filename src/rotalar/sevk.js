...
async function fisCacheYukle(fisNo, client, zorlaYenile = false) {
    ...
    console.log(`Sevk fişi cache yüklendi: ${fisNo} - ${kalemler?.length || 0} kalem, ${okunanQrler.size} okuma`);
...
router.get('/acik-fisler', async (req, res) => {
    try {
        ...
        const { data: fisler, error } = await client
            .from('sevk_fisi')
            .select('evrakno_seri, evrakno_sira, tarih, evrak_adi, miktar, paket_sayisi, cikis_depo, giris_depo')
