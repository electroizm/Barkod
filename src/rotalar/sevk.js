...
/**
 * GET /api/sevk/fis/:fisNo
 * Sevk fişi detaylarını getir (sadece Supabase)
 */
...
router.get('/fis-durumu/:fisNo', async (req, res) => {
    try {
        ...
        console.error('Sevk fişi durumu hatası:', error);
        return res.status(500).json({ success: false, message: 'Sunucu hatası: ' + error.message });
    }
});

/**
 * GET /api/sevk/okunan-qrler/:fisNo
    ...
    try {
        ...
        if (dbBirimPaket > 0 && paketToplam !== dbBirimPaket) {
            ...
            for (const kalem of eslesenKalemler) {
                ...
                if (guncelHata) {
                    console.error(`Sevk paket güncelleme hatası (id=${kalem.id}):`, guncelHata);
        ...
        console.error('Sevk fişi QR okutma hatası:', error);
        return res.json({ success: false, message: 'Sunucu hatası: ' + error.message, hata_tipi: 'SERVER_ERROR' });
    }
});

/**
 * GET /api/sevk/malzeme-paketler/:fisNo/:kalemId
...
router.get('/acik-fisler', async (req, res) => {
    try {
        ...
        const { data, error } = await client.rpc('sevk_acik_fisler_getir', { gun_sayisi: 17 });
...
router.get('/kapatilan-fisler', async (req, res) => {
    try {
        ...
        const { data, error } = await client.rpc('sevk_kapatilan_fisler_getir', { gun_sayisi: 17 });
