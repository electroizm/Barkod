...
function FisOkutmaOlustur(y) {
    ...
    async function fisYukle(no) {
        ...
        try {
            ...
            if (data.success) {
                ...
                el.fisOzet.textContent = 'Fi\u015f: ' + mevcutFis.evrakno_sira + ' - ' + (mevcutFis.evrak_adi || '-');
    ...
    async function durumGuncelle() {
        try {
            ...
            if (data.success) {
                if (data.fis) {
                    el.fisOzet.textContent = 'Fi\u015f: ' + data.fis.evrakno_sira + ' - ' + (data.fis.evrak_adi || '-');
                }
    ...
    function malzemeListesiGuncelle(kalemler) {
        ...
        var html = depoSirali.map(function(depoNo) {
            ...
            var depoAdi = depoNo === '0' ? 'Genel' : 'Depo ' + depoNo;
            ...
            if (data.success && data.fisler.length > 0) {
                el.acikFisListesi.innerHTML = data.fisler.map(function(fis) {
                    ...
                    return '<div class="fis-item" data-action="fisSecimAcik" data-fis="' + fis.evrakno_sira + '">' +
                        '<div class="fis-item-baslik">' + (fis.evrak_adi || y.varsayilanEvrakAdi) + ' - ' + fis.evrakno_sira + '</div>' +
