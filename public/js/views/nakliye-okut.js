/**
 * Barkod Okut View - Nakliye QR okutma sayfas\u0131
 * BarkodOkuyucu ile QR tarama, /api/supabase endpointleri.
 * Kamera g\u00fcvenli\u011fi: unmount() i\u00e7inde BarkodOkuyucu.destroy() ILK aksiyondur.
 */
window.Views = window.Views || {};
window.Views['nakliye-okut'] = (function() {
    var el = {};
    var _konteyner = null;
    ...
    function htmlOlustur() {
        return '' +
            ...
            '<div class="tamamlandi-kutu gizle" id="tamamlandiKutu">' +
                ...
                '<a href="/fis/nakliye-okutma" class="geri-btn">Oturum Listesine D\u00f6n</a>' +
            '</div>' +
    ...
    // === Cache ===
    function getCacheKey() { return 'qr_cache_' + oturumId; }
    ...
    async function frontendCacheSenkronize() {
        try {
            var r = await fetch('/api/supabase/okunan-qrler/' + encodeURIComponent(oturumId));
            var d = await r.json();
    ...
    async function qrOkut(qrKod) {
        ...
        try {
            ...
            var response = await fetch('/api/supabase/qr-okut', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ oturum_id: oturumId, qr_kod: qrKod, kullanici: kullanici })
            });
    ...
    async function durumGuncelle() {
        try {
            var response = await fetch('/api/supabase/okuma-durumu/' + encodeURIComponent(oturumId));
            var data = await response.json();
            if (data.success) {
    ...
    async function malzemeToggle(index, kalemId) {
        ...
        try {
            ...
            var r = await fetch('/api/supabase/malzeme-paketler/' + encodeURIComponent(oturumId) + '/' + kalemId);
            var d = await r.json();
    ...
    async function tumunuGenislet() {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (icerik && icerik.querySelector('.paket-yukleniyor')) {
                try {
                    var r = await fetch('/api/supabase/malzeme-paketler/' + encodeURIComponent(oturumId) + '/' + kid);
                    var d = await r.json();
                    if (d.success && d.paketler) {
                        icerik.innerHTML = d.paketler.map(function(p) {
                            var s = 'paket-gray';
    ...
    async function topluOkut(kalemId, malzemeAdi) {
        ...
        try {
            var kullanici = document.getElementById('kullaniciBilgi')?.textContent || 'bilinmiyor';
            var r = await fetch('/api/supabase/toplu-okut', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ oturum_id: oturumId, kalem_id: kalemId, kullanici: kullanici })
            });
            var d = await r.json();
            if (d.success) {
    ...
    function mount(konteyner, params) {
        ...
        // Oturum ID kontrolu
        oturumId = params && params.oturum;
        if (!oturumId) {
            AppRouter.git('/fis/nakliye-okutma');
            return;
        }
